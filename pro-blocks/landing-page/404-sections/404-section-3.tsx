...
const ActionLink = ({
  ...
  >
    <div className="bg-background flex items-center justify-center border border-border rounded-md w-10 h-10 shadow-sm">
      {Icon}
    </div>
    ...
    <ChevronRight className="size-5 hidden md:block transition-transform group-hover:translate-x-1" />
  </Link>
);
  ...
  return (
    ...
    >
      <div className="container px-6 flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
        ...
        <div className="flex-1 w-full p-3 md:p-6 bg-muted/40 rounded-xl flex flex-col gap-3">
          {ACTIONS.map((action) => (
